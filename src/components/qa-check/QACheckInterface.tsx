...
interface QACheckInterfaceProps {
  recordIds: string[];
...
export function QACheckInterface({ 
  recordIds,
  onRecordUpdate,
  onBack
}: QACheckInterfaceProps) {
  // Cache for loaded records
  const [recordCache, setRecordCache] = useState<Map<string, DatasetRecord>>(new Map());
  const [isLoading, setIsLoading] = useState(true);
  
  // Local status tracking for QA check
  const [localStatuses, setLocalStatuses] = useState<Record<string, 'pending' | 'approved' | 'rejected' | 'needs_review'>>({});

  // Batch load records on mount
  useEffect(() => {
    ...
    const fetchRecords = async () => {
      ...
      try {
        ...
        const initialStatuses: Record<string, 'pending' | 'approved' | 'rejected' | 'needs_review'> = {};
        ...
        for (let i = 0; i < recordIds.length; i += batchSize) {
          ...
          if (!error && data) {
            ...
            for (const row of data) {
              ...
              // Update local status from DB
              initialStatuses[row.id] = row.status as 'pending' | 'approved' | 'rejected' | 'needs_review';
            }
        ...
        toast.error('Lỗi khi tải records');
      } finally {
        if (!isCancelled) {
          setIsLoading(false);
        }
  ...
  const allSidebarItems = useMemo(() => {
    return recordIds.map(id => {
      ...
      return {
        ...
        status: localStatuses[id] || 'pending',
        landmark_name: cached?.metadata?.landmark_name || 'Loading...',
      };
    });
  }, [recordIds, recordCache, localStatuses]);
  ...
  const initialVisibleCount = useMemo(() => {
    const fivePercent = Math.ceil(recordIds.length * 0.05);
    return Math.max(100, Math.min(fivePercent, recordIds.length));
  }, [recordIds.length]);
  ...
  }, [filteredSidebarItems.length, recordIds.length]);
  ...
  // Calculate QA check stats from localStatuses
  const qaStats = useMemo(() => {
    const total = Object.keys(localStatuses).length;
    let approved = 0, pending = 0, rejected = 0, needs_review = 0;
    
    Object.values(localStatuses).forEach(status => {
      switch (status) {
  ...
  const updateRecordStatus = useCallback(async (recordId: string, status: 'pending' | 'approved' | 'rejected' | 'needs_review') => {
    try {
  ...
  const handleNeedsRecheck = async () => {
    ...
    if (toUpdate && currentRecordId) {
      await updateRecordStatus(currentRecordId, 'needs_review');
  ...
  const handleReject = async () => {
    const toUpdate = editedRecord || record;
    if (toUpdate && currentRecordId) {
      await updateRecordStatus(currentRecordId, 'rejected');
  ...
  const handleApprove = async () => {
    const toUpdate = editedRecord || record;
    if (toUpdate && currentRecordId) {
      await updateRecordStatus(currentRecordId, 'approved');
    ...
    return (
      <div className="h-full flex items-center justify-center bg-muted/30">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-muted-foreground">Đang tải {recordIds.length} records...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="h-full flex flex-col bg-muted/30">
        <div className="shrink-0 h-14 px-6 flex items-center justify-between border-b bg-background">
          <span className="font-semibold text-lg">Random QA Check</span>
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center space-y-4">
            <AlertTriangle className="h-16 w-16 mx-auto text-chart-4" />
            <h3 className="text-xl font-semibold">Không có dữ liệu</h3>
            <p className="text-muted-foreground">Vui lòng lấy mẫu trước khi kiểm tra</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="h-full flex flex-col bg-muted/30">
      ...
      <div className="shrink-0 h-14 px-6 flex items-center justify-between border-b bg-background">
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="font-semibold">Random QA Check (10%)</span>
          <span className="text-muted-foreground">•</span>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 text-xs">
            <span className="text-primary font-medium">{qaStats.approved} approved</span>
            <span className="text-muted-foreground">|</span>
            <span className="text-chart-4">{qaStats.needs_review} review</span>
            <span className="text-muted-foreground">|</span>
            <span className="text-destructive">{qaStats.rejected} rejected</span>
            <span className="text-muted-foreground">|</span>
            <span className="text-muted-foreground">{qaStats.pending} pending</span>
          </div>
      ...
      <div className="flex-1 min-h-0 flex">
        ...
        <div className="w-72 shrink-0 border-r bg-background flex flex-col">
          <div className="p-4 space-y-3">
            <div>
              <h3 className="font-semibold">QA Sample List</h3>
              <p className="text-xs text-muted-foreground">10% random từ dataset</p>
            </div>
          ...
          <div className="p-3 border-t text-xs text-muted-foreground">
            Showing {Math.min(visibleCount, filteredSidebarItems.length).toLocaleString()} / {filteredSidebarItems.length.toLocaleString()} (total: {recordIds.length.toLocaleString()})
          </div>
        </div>
