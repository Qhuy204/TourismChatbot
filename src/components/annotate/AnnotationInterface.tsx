...
export function AnnotationInterface({
  ...
  const handleScroll = useCallback(
    (e: React.UIEvent<HTMLDivElement>) => {
      ...
      if (scrollTop + clientHeight >= scrollHeight * 0.8) {
        setVisibleCount((prev) => Math.min(prev + 500, workingRecordIds.length));
      }
    },
  ...
  }, [initialRecordId, workingRecordIds.length]);
  ...
  }, [currentIndex, workingRecordIds.length]);
  ...
  const goNext = () => {
    if (currentIndex < workingRecordIds.length - 1) {
      ...
      <div className="shrink-0 h-14 px-6 flex items-center justify-between border-b bg-background">
        ...
        <div className="flex items-center gap-2">
          <Badge variant="outline" className="text-xs">
            {currentIndex + 1} / {workingRecordIds.length}
          </Badge>
        </div>
      </div>
        ...
        <div className="w-72 shrink-0 border-r bg-background flex flex-col">
          ...
          <ScrollArea className="flex-1" onScrollCapture={handleScroll}>
            <div className="p-2 space-y-1">
              {visibleSidebarItems.length === 0 && workingRecordIds.length === 0 ? (
              ...
              {visibleCount < workingRecordIds.length && (
          ...
          <div className="p-3 border-t text-xs text-muted-foreground">
            Showing {Math.min(visibleCount, workingRecordIds.length)} / {workingRecordIds.length} (total:{" "}
            {workingRecordIds.length})
          </div>
        </div>
            ...
            <div className="h-full flex flex-col bg-background">
              ...
              <div className="shrink-0 p-4 border-t flex items-center justify-between">
                ...
                <Button
                  ...
                  size="icon"
                  onClick={goNext}
                  disabled={currentIndex === workingRecordIds.length - 1}
                >
